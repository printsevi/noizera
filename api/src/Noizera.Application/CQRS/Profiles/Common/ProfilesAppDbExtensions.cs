...
internal static class ProfilesAppDbExtensions
{
    ...
    public static async Task<List<MusicSetQueryResult>> GetProfileFeaturedMusicSetsAsync(this AppDbContext db, string username, Guid? userId, CancellationToken ct)
    {
        FormattableString sql = $"""
                SELECT 
                    mc."PublicId" as PublicId,
