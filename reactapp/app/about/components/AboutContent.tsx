...
import { Headphones, Mail, Music2, Phone, Zap } from 'lucide-react'

export default function AboutContent() {
  return (
    <div className="p-4 sm:p-8">
      <motion.h1
        className="text-5xl font-bold text-center mb-8 text-purple-500"
      ...
      >
        We're transforming the music experience by connecting fans directly to independent artists. Our platform empowers creators with fair royalties and offers listeners unmatched FLAC-quality streaming. Join us in reshaping the future of music, where every play supports the talent you love.
      </motion.p>
      ...
      <div className="flex flex-col md:flex-row justify-center items-center gap-8 mb-16">
        <ContactButton icon={<Mail className="text-purple-500" />} text="hello@noizera.com" href="mailto:hello@noizera.com" />
        <ContactButton icon={<svg xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
...
function FeatureItem({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) {
  return (
    ...
    >
      ...
      <h3 className="text-xl font-semibold mb-2 text-purple-500">{title}</h3>
      <p className="text-gray-300">{description}</p>
    </motion.div>
  )
}

function ContactButton({ icon, text, href }: { icon: React.ReactNode, text: string, href: string }) {
  return (
    <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
      <Button asChild variant="outline" size="lg" className="gap-2 bg-transparent text-purple-500 border-purple-500 hover:bg-purple-500 hover:text-white">
